...
void calculate_energy(unsigned char *energy_out, const unsigned char *image_in, int width, int height, int cpp) {
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            ...
            /*
            int energy_r = sqrt((double)(red_x * red_x + red_y * red_y));
            int energy_g = sqrt((double)(green_x * green_x + green_y * green_y));
            int energy_b = sqrt((double)(blue_x * blue_x + blue_y * blue_y));

            int energy = (energy_r + energy_g + energy_b) / 3;
            */
            int index = (i * width + j);
